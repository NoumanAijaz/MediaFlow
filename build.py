...
def find_build_python(script_dir):
    ...
    if sys.executable and os.path.normcase(sys.executable).startswith(os.path.normcase(os.path.join(script_dir, "venv", ""))):
...
def ensure_output_unlocked(script_dir):
    ...
    try:
        ...
        try:
            ...
            pass
        pids = [parts[1] for line in running.splitlines()
                if (parts := line.split()) and len(parts) > 1 and parts[0].lower() == "mediaflow.exe"]
        print("\n" + "=" * 50)
